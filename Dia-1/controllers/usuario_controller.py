...
class LoginController(Resource):
    def post(self):
        ...
        try:
            ...
            # Lo convierto a bytes por que asi se trabaja la funcion checkpw
            ...
            if resultado:
                # identify > sirve para indicar a que usuario le pertenece a esta JWT
                token = create_access_token(identify = usuario_encontrado.id)
                return {
                    'content': token
                }
            else:
...
class PerfilController(Resource):
    # al poner ese deecorador ahora tiene que ser obligatorio el pasar el token
    @jwt_required()
    def get(self):
