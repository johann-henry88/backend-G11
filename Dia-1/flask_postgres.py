...
CORS(app, CORS_METHODS = ['GET', 'POST', 'PUT', 'DELETE'], origins= ['http://localhost:5000', 'http://127.0.0.1:5500'])


 # me conecto a la bd
conexion = connect(host='localhost', database='pruebas', user='postgres', password='powermusic')
...
def alumnos():
    if request.method == 'GET':
        ...
        print(data)
        cursor = conexion.cursor()
        cursor.execute("INSERT INTO ALUMNOS (nombre, apellido, maticulado) VALUES (%s, %s, %s)",(
    ...
    def gestion_alumno(id):
        if request.method == 'GET':
            ...
            # TODO: Recibir el id por la url y validar si el alumno existe , si existe, eliminarlo (hacer un delete) caso contrario 
