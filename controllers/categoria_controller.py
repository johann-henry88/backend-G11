...
class ImagenesController(Resource):
    def post(self):
        ...
        print(request.form)
        # Para obtener la llave que contengan archivos 'files'
        print(request.files)
...
class CategoriasController(Resource):
    def post(self):
        ...
        dto = CategoriaDto()
        # modifico la imagen agregando un identificador para evitar sobre escrituras
        nombre = secure_filename(uuid4().hex +'_'+ imagen.filename)

        # Agreggo a mi formulario el nombre de la imagen que me envia el cliente
