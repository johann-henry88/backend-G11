...
class ManejoUsuario(BaseUserManager):
    def create_superuser(self, correo, nombre, apellido, password, tipoUsuario):
        # este metodo se mandara a llamar cuando en la terminar se ponga 'python manage.py createsuperuser'
        if not correo:
...
class Usuario(AbstractBaseUser, PermissionsMixin):
    ...
    tipoUsuario = models.TextField(choices=[('ADMIN','ADMIN'), ('CLIENTE', 'CLIENTE')], 
    db_column='tipo_usuario')

    # campos netamente de auth_user
    # is_staff > sirve para indicar al panel administrativo que el usuario no pertenece al grupo de usuarios que puedn acceder
    ...
    # si queremos ingrear al panel administrativo tenemos que indicar que columnas usara para pedir el nombre de usuario
